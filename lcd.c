...
void LCD_Init (void)
{
	...
	send_byte (0xA6);//A6 normal , A7 reverse
	send_byte (0xA2);//Set bias 1/9 (Duty 1/65)
	
	
	//internal contrast voltage
	send_byte (0x2F);// power on 
	send_byte (0xF8);// boster ratio
	send_byte (0x00);// 4x
	//external contrast voltage
	//send_byte (0x2D);// power on
	
	send_byte (0x27);// v0 voltage regulator
	send_byte (0x81);// contrast
	send_byte (0x16);// 0x16
	send_byte (0xAC);// static indicator
	send_byte (0x00);// off
	send_byte (0xAF);// display on / 0xAE == off
